...
| `chat_id` | `STRING` | Previous chat id to continue the conversation. If this is the first message in the conversation, set it to `null`.
...
[NOTE]
The chat ID changes with every new message. You can follow each thread
...
Use `CALL ai.text.chat.providers()` (see xref:reference/functions-procedures.adoc#ai-text-completion-providers[reference]) to see supported providers and their configuration options in the installed version of the plugin.
