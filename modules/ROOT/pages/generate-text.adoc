...
[NOTE]
You can change the base URL used by the OpenAI provider (default: https://api.openai.com) via the `genai.openai.baseurl` setting.
This applies to all `ai.text.*` calls that use OpenAI, including `ai.text.embed` and `ai.text.embedBatch`.
