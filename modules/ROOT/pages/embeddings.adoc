...
For OpenAI, Azure OpenAI, and Vertex AI, the `maxBatchSize` configuration parameter can be used to limit the amount of data sent at a time to the provider.
If a single resource is larger than the specified `maxBatchSize`, it must be chunked manually before being passed to the procedure.
====
...
| `maxBatchSize`  | `INTEGER`| `8192`    | The maximum amount of data (measured in tokens) sent at a time. Introduced in 2026.04
...
| `maxBatchSize`  | `INTEGER`| -          | The maximum amount of data (measured in tokens) sent at a time. Introduced in 2026.04
