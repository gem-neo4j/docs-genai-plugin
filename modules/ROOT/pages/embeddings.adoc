...
[TIP]
To learn more about using embeddings in combination with vector indexes, see link:{neo4j-docs-base-uri}/cypher-manual/current/indexes/semantic-indexes/vector-indexes/#embeddings[Cypher -> Vector indexes -> Vectors and embeddings in Neo4j].
...
See identifiers under xref:reference/ai-providers.adoc[] for supported options.
...
.Create embeddings from a limited number of nodes and store them
====
...
<2> The link:{neo4j-docs-base-uri}/cypher-manual/current/values-and-types/lists/#cypher-list-comprehension[list comprehension] (`[]`) extracts the `title` and `plot` properties of the movies in `moviesList` into a new `LIST<STRING>`.
...
[#encodebatch-large-dataset]
.Create embeddings from a large number of nodes and store them
====
[source, cypher]
----
MATCH (m:Movie WHERE m.plot IS NOT NULL)
...
Note that this `CALL` subquery uses a link:{neo4j-docs-base-uri}/cypher-manual/current/subqueries/call-subquery/#variable-scope-clause[variable scope clause] to import variables.
...
<7> Set to `1` the amount of batches to be processed at once (see link:{neo4j-docs-base-uri}/cypher-manual/current/subqueries/subqueries-in-transaction/#concurrent-transactions[`CALL` subqueries -> Concurrent transactions]).
